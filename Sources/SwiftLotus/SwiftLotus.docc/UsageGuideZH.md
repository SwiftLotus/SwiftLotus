...
这份文档从安装、TCP 服务开发、连接管理、运行时管理到生产部署，说明 SwiftLotus 的主要使用方式。
...
根包只包含网络核心。Redis、MySQL、Postgres 适配器位于 `Addons/`，需要时再单独引入。
...
这个例子会启动一个常驻 TCP worker，管理连接生命周期，并通过 group 广播消息。
...
如果协议需要登录态，可以启用认证超时，并在认证成功后标记连接：
...
普通业务建议使用 `onMessage`，可以自然调用数据库、上游服务或 actor：
...
热点路径如果只需要构造小响应并立刻写回，可以使用 `onMessageSync`，避免每条消息创建 Swift task：
...
WebSocket 网关可以使用 ``WebSocketProtocol``：
...
需要连接上游 TCP 服务时，使用 ``AsyncTcpConnection``：
...
需要出站 HTTP 调用时，使用 ``SwiftLotusHTTPClient``：
...
需要 manager 常驻并自动拉起退出的 worker 时，使用 `supervise`：
...
SwiftLotus 会记录连接创建/关闭、当前连接数、收发消息数、收发字节数、背压事件和错误数。业务也可以写入自定义指标：
...
生产环境中可以定期把 snapshot 导出到自己的监控系统。
...
- 设置写缓冲水位，并在背压过高时暂停读取。
- 需要自动拉起 worker 时使用 `supervise`。
- 需要平滑替换 worker 时使用 `rolling-reload`。
- 对 reloadable worker 使用 `reload` 或 `SIGUSR1` 触发重载。
- 将 `worker.status` 和 `worker.metrics.snapshot()` 接入运维系统。
- 修改协议热路径后，运行 `Benchmarks/TCP` 和 `Benchmarks/HTTP` 做本机回归压测。
...
这些 benchmark 适合开发阶段回归，不等同于行业排行。正式 HTTP 框架横向对比建议使用 TechEmpower Framework Benchmarks。
